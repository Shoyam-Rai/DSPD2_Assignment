...
void insertSorted(int id, const char *name, int total_steps) {
    ...
    if (ind_step_list == NULL || total_steps > ind_step_list->total_steps) {
        newNode->next = ind_step_list;
        ind_step_list= newNode;
    } else {
...
void Get_top_3() {
    ...
    while (node != NULL) {
        ...
        if (allGreater) {
            ...
            insertSorted( node->id, node->name, total_steps);
        }
...
void insertSorted2( const char *group_name, int total_steps) {
    ...
    if (grp_step_list == NULL || total_steps > grp_step_list->total_steps) {
        newNode->next = grp_step_list;
        grp_step_list = newNode;
    } else {
        LeaderboardNode *current = grp_step_list;
        while (current->next != NULL && total_steps <= current->next->total_steps) {
            current = current->next;
        }
        newNode->next = current->next;
...
void Get_leader_head() {
    ...
    while (groupList != NULL) {
        ...
        for (int i = 0; i < groupList->group_size; i++) {
         ...
         insertSorted2( groupList->group_name, totalGroupSteps);
